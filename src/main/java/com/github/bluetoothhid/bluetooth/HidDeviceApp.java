...
public class HidDeviceApp {
    ...
    public void sendReport(Report report) {
        if (inputHost != null && device != null) {
            inputHost.sendReport(device, report.getID(), report.build());
        }
    }
    ...
    @BinderThread
    private boolean replyReport(BluetoothDevice device, byte type, @ID byte id) {
        if (id != ID.KEYBOARD) {
            return false;
        }

        if (inputHost != null) {
            inputHost.replyReport(device, type, id, new byte[8]);
        }
        return true;
    }
